...
def update(self):
    if self.alive:
        if self.facing == "R":
            ...
            if len(pygame.sprite.spriteecollide(self,self.level.walls, False)) >=1:
                ...
                if self.level.chest is not None and pygame.sprite.collide_rect(self, self.level.chest):
                    self.level.message.add(MovingMessage(self.level.chest.give_bonus(self), self.level.chest.rect.center))
                else:
                    self.movement[0] *= -1
                    self.movement[1] *= -1
                    self.rect.move_ip[self.movement]
                self.enemies = pygame.sprite.spritcollide(self, self.level.enemies, False)
                self.movement[0]= 0
                self.movement[1]= 0
def attack (self):
    if len(self.enemies) > 0:
        random.choices(self.enemies).defend(self.attack_damage)
...
def defend(self, damage):
    ...
    if self.health<= 0:
        ...
        self.level.floor.add(Title("image/monsters/grave.gif", (self.rect.x-15, self.rect.y-25)))
...
def change_x(self,change):
    ...
    if change > 0:
        ...
        self.facing = "L"
def change_y(self,charge):
    self.movement[1]= charge
def save(self):
    self.image= None
    self.level= None
    self.enemies= None
